...
- [ ] **0.2** Startup'taki otomatik `admin/admin123` hesabı oluşturma kodunu kaldır (`backend/server.py:1197-1210`). İlk kurulumda env'den okunan tek seferlik setup token'ı ile yapılacak.
- [ ] **0.3** Public `/seed` endpoint'ini kaldır veya `if not PROD` ile koru (`backend/server.py:1115-1124`).
- [ ] **0.4** **Sipariş toplamını backend'de yeniden hesapla.** Client'tan gelen `total` ve `price` field'larına güvenme. Item'ları DB'den çek, fiyatı oradan al, total'i backend hesapla (`backend/server.py:519`, `OrderItem` modeli `server.py:101`).
...
- [ ] **1.1** `JWT_SECRET` hardcoded default'unu kaldır (`backend/server.py:25`). Env yoksa app başlamasın (fail-fast).
